...
from models.stage2_retnet.retention import MultiScaleRetention
...
class RetNet(nn.Module):
    ...
    def forward(self, X):
        ...
        for i in range(self.layers):
            # print('self.embedding(X).transpose(1,2):',self.embedding(X).transpose(1,2).shape,h[:, :, None].shape)
            Y = self.retentions[i](self.layer_norms_1[i](X)) + X
            X = self.ffns[i](self.layer_norms_2[i](Y)) + Y
            # X = self.layer_norms_2[i](Y) + Y
        return X
